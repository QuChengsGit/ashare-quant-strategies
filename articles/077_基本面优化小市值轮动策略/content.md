...
**基本面优化小市值轮动策略** 是一种基于市值和基本面因子的量化选股策略。策略的核心思想是通过筛选基本面良好且市值较小的股票，利用其高成长性与市场波动性较大的特性，在适当的时机进行轮动操作，从而实现超额收益。策略同时设置了动态持仓、选股频率、以及止损机制，以优化投资组合的表现和控制风险。
...
1\. 策略初始化 (initialize)
  ...
  * **功能** : 初始化策略的各项参数，包括基准指数、滑点、交易费用等设置，并初始化全局变量和容器。

  * **配置** :

    * set_benchmark('000300.XSHG'): 设置沪深300为基准指数。

    * set_slippage(FixedSlippage(0.01)): 设置固定滑点为0.01。

    * set_order_cost(OrderCost(...), type='stock'): 设置股票的交易费用。

2\. 动态仓位与频率初始化 (check_dynamic_initialize)

  * **功能** : 设置策略的动态参数，包括个股最大持仓比重、选股频率、买入卖出频率等。

  * **配置** :

    * g.security_max_proportion: 个股最大持仓比重，默认1（即100%）。

3\. 股票池初筛 (check_stocks_initialize)

  * **功能** : 设置股票池初筛条件，包括是否过滤停牌、退市、ST股票，行业及概念过滤，以及黑名单股票。

  * **配置** :

    * g.security_universe_index = ['all_a_securities']: 设定股票池为全部A股。

4\. 选股主函数 (main_stock_pick)

  * **功能** : 通过基本面因子筛选出符合条件的股票列表，并根据市值进行排序，生成买卖股票列表。

  * **逻辑** :

    * 通过查询并筛选市值较小、基本面良好的股票，确定买入和卖出列表。

5\. 买卖操作函数 (trade)

  * **功能** : 根据生成的买卖列表执行相应的交易操作。

  * **逻辑** :

    * 定期买入符合条件的股票，卖出不再符合条件的股票。

6\. 辅助函数

  * **paused_filter** : 过滤停牌股票。

  * **delisted_filter** : 过滤退市股票。

  * **st_filter** : 过滤ST股票。

  * **high_limit_filter** : 过滤涨停股票。

  * **get_security_universe** : 获取指定范围的股票池。

  * **industry_filter** : 过滤特定行业的股票。

  * **concept_filter** : 过滤特定概念的股票。

  * **selled_security_list_count** : 统计已卖出股票的天数。

### 策略亮点

  * **基本面筛选** : 通过筛选市值较小且基本面良好的股票，增强了投资组合的成长性和安全性。

  * **动态调仓** : 通过设定动态的持仓比例和调仓频率，策略能够适应市场变化，及时调整投资组合。

  * **风险控制** : 引入了多个过滤条件和止损机制，有效控制投资风险。
