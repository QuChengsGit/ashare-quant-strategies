...
该策略利用多因子筛选模块，基于股息率、波动率、流动性、财务状况等多维度因子对股票进行筛选和排序，结合自定义交易规则实现轮动投资。通过定期调仓、风险控制和智能交易模块，策略旨在提高收益率并降低回撤。
...
## 3.1 策略逻辑
  ...
  * **因子筛选** ：通过多种财务因子（ROA、留存收益率、非线性市值）筛选高质量股票。

  * **调仓机制** ：每周根据因子得分调整持仓，卖出不符合要求的股票，并买入符合条件的股票。

  * **风险控制** ：过滤ST股、次新股以及涨停和跌停的股票，以防止高风险资产进入组合。

3.2 交易机制

  * **开仓与平仓** ：根据每周筛选的股票列表进行开仓，已经持有的股票在符合条件时平仓。

# 4. 策略特点

  * **多因子筛选** ：基于多因子模型，精选成长性好、估值合理的股票，确保策略稳健性。

  * **自动轮动** ：策略自动化进行持仓调整，保证组合的动态优化。

关注**微信公众号：** ，底部回复“**量化** ”，加入**AI量化交易共学小组微信群** ，赠送8G量化交易学习资料，每周在小组内部腾讯会议线上分享一个量化交易策略的开发过程。
